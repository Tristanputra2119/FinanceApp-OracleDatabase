    ...
    <div class="auth-card">
      <div style="display:flex; align-items:center; gap:10px; margin-bottom:1.5rem;">
        ...
        <div>
          ...
          <p style="color:#64748b; font-size:12px;">Create a new account</p>
        </div>
      </div>

      <div v-if="isOnline === false" style="background:rgba(234,179,8,0.1);border:1px solid rgba(234,179,8,0.3);border-radius:8px;padding:10px 12px;margin-bottom:16px;color:#fbbf24;font-size:13px;display:flex;align-items:center;gap:8px;">
        <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
        </svg>
        Backend offline — registration unavailable
      </div>

      <div v-if="authStore.error" style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:10px 12px;margin-bottom:16px;color:#fca5a5;font-size:13px;">
        {{ authStore.error }}
      </div>

      <form @submit.prevent="submit" style="display:flex; flex-direction:column; gap:14px;">
        <div>
          <label style="display:block; font-size:13px; color:#94a3b8; margin-bottom:6px;">Full Name</label>
        ...
        <div>
          ...
          <div style="display:flex; gap:4px; margin-top:6px;">
            <div :style="{ flex:1, height:'3px', borderRadius:'2px', background: password.length >= 1 ? '#ef4444' : '#334155', transition:'background 0.2s' }"></div>
            <div :style="{ flex:1, height:'3px', borderRadius:'2px', background: password.length >= 4 ? '#eab308' : '#334155', transition:'background 0.2s' }"></div>
            <div :style="{ flex:1, height:'3px', borderRadius:'2px', background: password.length >= 8 ? '#22c55e' : '#334155', transition:'background 0.2s' }"></div>
          </div>
        </div>
      ...
      <p style="text-align:center; margin-top:1.25rem; color:#64748b; font-size:13px;">
        Have an account?
        <router-link to="/login" style="color:#60a5fa; text-decoration:none; font-weight:500;"> Sign in</router-link>
      </p>
    </div>
  </div>
</template>
